...
const loadClients = async () => {
  try {
    ...
    // Vérifier si projets est un tableau
    const userClients = data.filter(client => 
      client.projets && client.projets.some(projet => projet.user_id === userId)
    )

    setClients(userClients) // Stocker les clients filtrés
    setFilteredClients(userClients) // Initialiser les clients filtrés
    console.log("Clients chargés :", userClients) // Vérification des données filtrées
  } catch (error) {
    console.error("Erreur lors du chargement des clients :", error)
  }
}

// Charger les clients une fois que userId est défini
useEffect(() => {
  if (userId) {
    loadClients()
  }
